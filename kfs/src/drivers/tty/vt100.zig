// The VT100 escape sequences a console understands. Split out for size only:
// these reach into the console's scrollback and cursor directly.

const std = @import("std");

const VtConsole = @import("vt_console.zig");

const history_size = VtConsole.history_size;

/// handle move escape codes
fn handle_move(terminal: *VtConsole, buffer: [:0]const u8) void {
...
fn handle_set_attribute(terminal: *VtConsole, buffer: [:0]const u8) void {
    const len = std.mem.indexOfSentinel(u8, 0, buffer);
    const n: u32 = std.fmt.parseInt(u32, buffer[1 .. len - 1], 10) catch 0;
    switch (n) {
        @intFromEnum(VtConsole.Attribute.reset) => {
            terminal.attributes = 0;
            terminal.reset_color();
        },
        @intFromEnum(VtConsole.Attribute.bold)...@intFromEnum(VtConsole.Attribute.hidden) => {
            terminal.attributes |= @as(u16, 1) << @intCast(n);
        },
        30...37 => {
            terminal.set_font_color(switch (n) {
                30 => VtConsole.Color.black,
                31 => VtConsole.Color.red,
                32 => VtConsole.Color.green,
                33 => VtConsole.Color.yellow,
                34 => VtConsole.Color.blue,
                35 => VtConsole.Color.magenta,
                36 => VtConsole.Color.cyan,
                37 => VtConsole.Color.white,
                else => VtConsole.Color.white,
            });
        },
        40...47 => {
            terminal.set_background_color(switch (n) {
                40 => VtConsole.Color.black,
                41 => VtConsole.Color.red,
                42 => VtConsole.Color.green,
                43 => VtConsole.Color.yellow,
                44 => VtConsole.Color.blue,
                45 => VtConsole.Color.magenta,
                46 => VtConsole.Color.cyan,
                47 => VtConsole.Color.white,
                else => VtConsole.Color.black,
            });
        },
        else => {},
    }
...
fn handle_save(terminal: *VtConsole, buffer: [:0]const u8) void {
    const saved_state = struct {
        var value: ?VtConsole.State = null;
    };
    switch (buffer[0]) {
...
fn handle_clearline(terminal: *VtConsole, buffer: [:0]const u8) void {
    const len = std.mem.indexOfSentinel(u8, 0, buffer);
    const n: i32 = std.fmt.parseInt(i32, buffer[1 .. len - 1], 10) catch 0;
    switch (n) {
        0 => {
            for (terminal.pos.col..VtConsole.width) |i| {
...
fn handle_clearscreen(terminal: *VtConsole, buffer: [:0]const u8) void {
    const len = std.mem.indexOfSentinel(u8, 0, buffer);
    const n: i32 = std.fmt.parseInt(i32, buffer[1 .. len - 1], 10) catch 0;
    const screen_bottom = (terminal.head_line + history_size - terminal.scroll_offset) % history_size;
    var screen_top = (screen_bottom + history_size - VtConsole.height + 1) % history_size;
...
fn handle_goto(terminal: *VtConsole, buffer: [:0]const u8) void {
    ...
    const off_v: i32 = -@as(i32, @intCast(terminal.pos.line)) +
        @mod(@as(i32, @intCast(terminal.head_line)) -
            ...
            @as(i32, @intCast(VtConsole.height - 1)) +
...
fn handle_get_pos(terminal: *VtConsole, _: [:0]const u8) void {
    ...
    terminal.tty.input(slice);
}

fn handle_nothing(_: *VtConsole, _: [:0]const u8) void {}
...
pub fn handle_escape(terminal: *VtConsole, buffer: [:0]const u8) error{InvalidEscape}!bool {
    const map = [_]struct {
        prefix: [:0]const u8,
        f: *const fn (*VtConsole, buffer: [:0]const u8) void,
    }{
        ...
        .{ .prefix = "#5", .f = &handle_nothing }, // Single width, single VtConsole.height letters
        .{ .prefix = "#6", .f = &handle_nothing }, // Double width, single VtConsole.height letters
