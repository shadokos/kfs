...
pub fn do(path1: [*:0]const u8, path2: [*:0]const u8) Errno!void {
    const path_slice = std.mem.span(path1);
    const target_path_slice = std.mem.span(path2);
    ...
    const target = try vfs.resolve(target_path_slice);
    defer target.release();

    if (target.inode.superblock != dir_tnode.inode.superblock) {
        return Errno.EXDEV;
    }

    try dir_tnode.inode.link(name, target.inode);
}
