...
pub fn load_inode(self: *Self, ino: VfsInode.Ino) (error{ENOMEM} || ReadError)!*Inode {
    const inode = try Inode.create();
    try inode.init_and_fetch(self, ino);
    return inode;
}

pub fn retrieve_inode(self: *Self, ino: VfsInode.Ino) VfsSuperblock.Error.load_inode!*Inode {
...
pub fn erase_bytes(self: *Self, nbytes: usize, block: usize, offset: usize) WriteError!void {
    var block_buffer: []u8 = self.allocator.alloc(u8, self.vfs.block_size) catch @panic("todo");
    defer self.allocator.free(block_buffer);
    var current_block = block;
    var src_pos: usize = 0;
    var file_offset = offset;
    while (src_pos < nbytes) {
        try self.read_block(current_block, block_buffer);
        const partial_begin: usize = file_offset % self.vfs.block_size;
        const partial_end: usize = @min(self.vfs.block_size, partial_begin + nbytes - src_pos);

        @memset(block_buffer[partial_begin..partial_end], 0);
        try self.write_block(current_block, block_buffer); // todo: return partial in case of error
