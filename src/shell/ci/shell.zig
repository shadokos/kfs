...
fn apic_handler(_: interrupts.InterruptFrame) void {
    apic.ack(.COM1);
}
...
pub fn on_init(shell: *Shell) void {
    ...
    interrupts.set_trap_gate(apic.IRQ.COM1, interrupts.Handler.create(&apic_handler, false));
    apic.enable_irq(apic.IRQ.COM1);
