...
pub const Cache = struct {
    ...
    obj_per_slab: paging.obj_idx_t = 0,
    size_obj: usize = 0,
    align_obj: usize = 0,
    ...
    pub fn init(
        ...
        const obj_per_slab = total / new.slot_size;

        if (obj_per_slab == 0 or obj_per_slab > std.math.maxInt(paging.obj_idx_t))
            return Error.InitializationFailed;

        new.obj_per_slab = @intCast(obj_per_slab);
    ...
    pub fn unsafe_free(_: *Self, slot_ptr: *usize) Slab.Error!void {
        ...
        if (decoded == 0 or (decoded >= base and decoded < slab_end and (decoded - base) % cache.slot_size == 0)) {
            if (try cache.walk_freelist(slab, slot_addr)) return Slab.Error.DoubleFree;
        }

        if (cache.debug.sanity) _ = try cache.walk_freelist(slab, null);
    ...
    fn walk_freelist(self: *Cache, slab: Slab, target: ?usize) Slab.Error!bool {
        const base = slab.base_addr();
        const slab_end = base + self.slot_size * self.obj_per_slab;

        var cursor = slab.next_free();
        var count: paging.obj_idx_t = 0;
        ...
        while (cursor != 0) : (count += 1) {
            ...
            if (target) |t| if (cursor == t) return true;
