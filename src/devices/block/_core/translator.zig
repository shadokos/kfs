...
pub const DirectTranslator = @import("translators/direct_translator.zig");
pub const ScaledTranslator = @import("translators/scaled_translator.zig");
...
pub fn createTranslator(allocator: std.mem.Allocator, physical_block_size: u32) !*BlockTranslator {
    if (physical_block_size == STANDARD_BLOCK_SIZE) {
        const direct = try DirectTranslator.create(allocator, physical_block_size);
        return &direct.base;
    } else {
