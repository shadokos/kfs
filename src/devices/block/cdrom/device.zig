...
const logger = std.log.scoped(.blockdev_cd);
...
fn generateDeviceName() ![16]u8 {
    ...
    _ = try std.fmt.bufPrint(&name, "cd{}", .{created_count});
...
pub fn read(self: *Self, lba: u32, count: u16, buffer: []u8) BlockError!void {
    ...
    ide.performOperation(.ATAPI, &op) catch |err| {
        self.stats.errors += 1;
...
fn flush(_: *BlockDevice) BlockError!void {
    // TODO: Implements the flush cache command for cdrom
