...
pub const Utf8ToCp437Iterator = struct {
    ...
    pub fn next(self: *@This()) ?u8 {
        ...
        if (iter.nextCodepoint()) |codepoint| {
            ...
            const len = std.unicode.utf8CodepointSequenceLength(codepoint) catch 1;
            self.index += len;

            // Search in the mapping table
            var buf: [4]u8 = undefined;
            const utf8_char = buf[0 .. std.unicode.utf8Encode(codepoint, &buf) catch return '?'];

            if (unicode_to_cp437.get(utf8_char)) |cp437_code| {
                return cp437_code;
            }

            // If not found and it's in extended ASCII range, keep it
            if (codepoint < 256) {
